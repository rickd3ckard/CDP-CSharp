...
namespace CDP.Objects
{
    public class DOM
    {
        ...
        public async Task<RemoteObject> ResolveNode(int NodeId, TimeSpan? TimeOut = null)
        {
            if (_document == null) { throw new NullReferenceException(); }
            if (TimeOut == null) { TimeOut = TimeSpan.FromSeconds(10); }
            Stopwatch stopWatch = Stopwatch.StartNew();
            ...
            int commandId = 1;
            await this.WebSocket.SendAsync(new DOMResolveNodeCommand(commandId, NodeId).Encode(),
                WebSocketMessageType.Text, true, CancellationToken.None);
            ...
            while (true)
            {
                ...
                if (result.EndOfMessage == true)
                {
                    ...
                    JsonElement nodeElement = commandResult.Result.RootElement.GetProperty("object");
                    RemoteObject? remoteObject = JsonSerializer.Deserialize<RemoteObject>(nodeElement.GetRawText());
                    if (remoteObject == null) { throw new InvalidCastException(); }
                    return remoteObject;
                }
            }
        }
